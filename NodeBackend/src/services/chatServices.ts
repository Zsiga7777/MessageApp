...
export async function getChatsByUserServiceAsync(userId: Types.ObjectId) {
  return await chatModel.find({ users: userId });
}

export async function getChatWithUsersServiceAsync(chatId: Types.ObjectId) {
  return (await chatModel.findOne({ _id: chatId })).populated("users").exec();
}

export async function createChatServiceAsync(chatData: Partial<IChat>) {
  try {
...
export async function updateChatByIdServiceAsync(
  _id: Types.ObjectId,
...
export async function deleteChatByIdServiceAsync(id: Types.ObjectId) {
